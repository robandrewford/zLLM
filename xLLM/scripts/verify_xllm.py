...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("data/logs/xllm_verification.log"),
...
def load_xllm():
    ...
    try:
        ...
        kb_path = Path("data/knowledge/knowledge_base.pkl")
        if kb_path.exists():
            logger.info(f"Loading knowledge base from {kb_path}")
            kb.load(kb_path)
        else:
            test_kb_path = Path("data/knowledge/test/knowledge_base.pkl")
...
def verify_knowledge_base_construction(verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        expected_attrs = ["dictionary", "embeddings", "hash_related", "hash_category", "hash_see"]
...
def verify_query_processing(xllm_module, queries: List[str], verbose: bool = False) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument("--test-queries-file", type=str, help="Path to a file containing test queries")
        ...
        try:
            with open(args.test_queries_file, "r") as f:
                ...
                if file_queries:
                    ...
                    logger.warning(f"No queries found in {args.test_queries_file}, using default queries")
        except Exception as e:
    ...
    report_path = Path("data/logs/verification_report.txt")
