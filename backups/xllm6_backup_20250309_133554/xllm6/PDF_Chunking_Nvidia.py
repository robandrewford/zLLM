...
def detect_table(xLLM_entity):
    ...
    for k in range(1, len(entity_type)):
        ...
        if (
            (
                (type == "Data" and old_type == "Note")
                or (type == "Note" and old_type == "Data")
                or (type == "Data" and old_type == "Data")
            )
...
def cprint_page(xLLM_entity, OUT):
    ...
    for k in range(len(entity_type)):
        ...
        OUT.write(
            f"{type:<8}{block_ID:>3}{item_ID:>5}{sub_ID:>3}{pn:>3}"
...
def update_page(text, type, entity, idx):
    ...
    if (
        type == old_type
        and block_ID == old_block_ID
        and item_ID == old_item_ID
        and sub_ID == old_sub_ID
    ):
...
def convert_pdf_to_json(pdf_path, json_path, doc_ID, text_path):
    ...
    for page_num in range(len(pdf_document)):
        ...
        text_data = page.get_text(
            "dict"
        )  # also extract as "json" to get tokens in green font
        ...
        for tabs_index, tab in enumerate(tabs):
            ...
            table_data = (
                tab.extract()
            )  # extracting tabs[i], the i-th table in this page
            if len(table_data) > 0:
                ...
                for line in block["lines"]:
                    for span in line["spans"]:
                        ...
                        if block_ID == -1:
                            ...
                            if (
                                old_text != ""
                                and ord(old_text[0]) != 8226
                                and type not in ("List", "SubList")
                            ):
                                block_ID += 1
