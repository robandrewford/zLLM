...
compressed_ngrams_table = llm6.read_table(
    "xllm6_compressed_ngrams_table.txt", type="list", path=""
)
compressed_word2_hash = llm6.read_table(
    "xllm6_compressed_word2_hash.txt", type="hash", path=""
)
embeddings = llm6.read_table(
    "xllm6_embeddings.txt", type="hash", path="", format="float"
)
embeddings2 = llm6.read_table(
    "xllm6_embeddings2.txt", type="hash", path="", format="float"
)
...
if build_taxonomy_tables:
    ...
    taxonomy_tables = create_taxonomy_tables(
        threshold, thresh2, ignoreWords, dictionary
    )
    ...
    connectedTopWords = dict(
        sorted(connectedTopWords.items(), key=lambda item: item[1], reverse=True)
    )
    ...
    connectedByTopWord = llm6.read_table(
        "xllm6_connectedByTopWord.txt", type="hash", path=""
    )
