...
def update_tables(backendTables, word, hash_crawl, backendParams):
    ...
    update_nestedHash(
        backendTables["hash_context4"], word, description
    )  # takes space, don't build?
    ...
    update_nestedHash(
        backendTables["full_content"], word, full_content
    )  # takes space, don't nuild?
            ...
            if word not in stopwords:
                ...
                for k in range(1, max_multitoken):
                    if position > k:
                        ...
                        update_hash(
                            hwords, key
                        )  # for word correlation table (hash_pairs)
                        update_tables(backendTables, word, hash_crawl, backendParams)
...
def print_results(q_dictionary, q_embeddings, backendTables, frontendParams):
    ...
    if show["Embeddings"]:
        ...
        q_embeddings = dict(
            sorted(q_embeddings.items(), key=lambda item: item[1], reverse=True)
        )
        print()
        print(
            "%3s %s %1s %s %s"
            % (
                ...
                "pmi".ljust(4),
                "F",
                "token [from embeddings]".ljust(35),
        ...
        for key in q_embeddings:
            ...
            if keyAB in ctokens:
                flag = "*"
            if (
                ntk1 >= frontendParams["embeddingKeyMinSize"]
                and ntk2 >= frontendParams["embeddingValuesMinSize"]
                and pmi >= frontendParams["min_pmi"]
                and nAB >= frontendParams["nABmin"]
                and token not in local_hash
    ...
    for label in show:
        ...
        if show[label] and label in sectionLabels:
            ...
            for word in q_dictionary:
                ...
                if (
                    word not in ignore
                    and ntk3 >= frontendParams["ContextMultitokenMinSize"]
                ):
