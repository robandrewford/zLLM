    ...
    def _crawl_page(self, url):
        ...
        for circuit_retry in range(self.circuit_retries):
            ...
            if circuit_retry < self.circuit_retries - 1:
                logger.info(f"Renewing Tor circuit and trying again...")
                self._renew_tor_circuit()

        # If we've exhausted all retries and circuit rebuilds
        logger.error(f"Failed to crawl {url} after {self.max_retries} retries and {self.circuit_retries} circuit rebuilds")
        return None
...
def main():
    ...
    parser.add_argument("--tor-proxy", type=str, default="socks5://127.0.0.1:9050",
    ...
    parser.add_argument("--circuit-retries", type=int, default=2,
